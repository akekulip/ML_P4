...
def report() -> None:
    ...
    for s in seeds:
        ...
        owned = np.isin(oc, (1, 2))                       # OWNER, NEW_OWNER
        seen = pd.Series(owned.astype(int)).groupby(f).cumsum().to_numpy() - owned
        lost = pd.Series((oc == 2) & (seen > 0)).groupby(f).any()   # took a slot again after holding one
        down = coll | lost
    ...
    med, gmin, gmax = df.gap_down.median(), df.gap_down.min(), df.gap_down.max()
    ...
    for r in rows:
        L.append(f"| {r['seed']} | {r['downgraded_flows']:,} | {r['downgraded_flow_share']:.4%} | "
                 f"{r['downgraded_packet_share']:.4%} | {r['flows_with_empty_refusal']:,} | {r['gap_down']:+.4f} |")
    L += ["", ("A flow is downgraded when an active incumbent refused it a slot (collision) or a flow lost its slot and took one again. Empty-slot refusals are "
          ...
          f"| downgraded flows only, per-seed gap | median {med:+.4f} | range {gmin:+.4f} to {gmax:+.4f} | |",
    ...
    g = med
    L.append("The per-seed gap is bimodal (seeds that downgrade a few very large flows give a much larger gap), so the "
             "verdict uses the median, not the mean. This measures only the benign-only population on PeerRush, one victim task. "
